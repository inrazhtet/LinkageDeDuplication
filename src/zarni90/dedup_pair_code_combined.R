...
#### Get data
####
load("~/git/dhs_link_char/data/dhs_link_char/final/eto_final.RData")
load("~/git/dhs_link_char/data/dhs_link_char/final/wvs_final.RData")
...
nrow(unique(wvs_dmg_prsd)) #58115
...
nrow(matched_system_ids) #25745 Rows
...
demo_dedup_output <- matrix(NA, 1, ncol = ncol(wvs_dmg_prsd))
...
for (i in 1:length(matched_system_ids_list)){
  results_perfect <- as.data.frame(wvs_dmg_prsd[c(unique(unlist(strsplit(matched_system_ids_list[i], ",")))) , ])
...
save(demo_dedup_output, file="./data/dhs_link_char/working/wvs_sameid_dedup_21_50.RData")
...
#####################################################################################
extract=getPairs(fspairs, min.weight=40, max.weight=80, single.rows = TRUE)
View(extract)
...
save(imperfect_matched_ids_group, file = "./data/dhs_link_char/final/wvs_systemkeys_to_match.Rdata")

#This is the System ID pairs that are the same.
imperfect_ids <- unique(imperfect_matched_ids_group$pairs)
length(imperfect_ids) #70 rows of it # But there are many ids inside
#imperfect_ids[1:10]

wvs_dmg_prsd <- as.data.frame(wvs_dmg_prsd)
imperfect_output <- matrix(NA, 1, ncol = ncol(wvs_dmg_prsd))
for (i in 1:length(imperfect_ids)){
  #Should we bother to only put unique system keys in when the index will expand it back up
  results_imper <- as.data.frame(wvs_dmg_prsd[which(wvs_dmg_prsd$id %in% unique(unlist(strsplit(imperfect_ids[i], ",")))) , ])
  test <- results_imper
  #View(results_imper)
...
save(imperfect_output, file="./data/dhs_link_char/working/wvs_imperfect_matches_dem_50_80.RData")
...
wvs_unmerged <- anti_join(wvs_dmg_prsd, dededup_all_ids, by = c("id"))

#GET EVERYTHING ELSE THAT IS NOT FROM THE LIST (THE MAJOR LIST)

wvs_final <- rbind(dedup_final_merged, wvs_unmerged)

nrow(wvs_final) #43332
nrow(wvs_dmg_prsd) #64927
